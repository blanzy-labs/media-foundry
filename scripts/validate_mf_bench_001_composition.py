...
def main() -> int:
    ...
    composition_path = root / "config/mf-bench-001-composition.json"
    config = json.loads(config_path.read_text())
    composition = json.loads(composition_path.read_text())
    art = root / "artifacts/mf-bench-001"
    checks = {}
    ...
    paths = [art / "blockout" / f"{label}.png" for label in labels]
    ...
    contract_path = art / "blockout/scene-contract.json"
    contract = json.loads(contract_path.read_text()) if contract_path.is_file() else {}
    fingerprint = contract.get("render_fingerprint", {})
    fingerprint_ok = fingerprint.get("config_sha256") == sha256(config_path) and fingerprint.get("builder_sha256") == sha256(root / config["render"]["blender"]["builder_script"]) and fingerprint.get("template_sha256") == sha256(root / config["render"]["blender"]["template"])
    ...
    report = {"slice": "MF-BENCH-001", "result": result, "state": state, "passed": len(checks) - len(failures), "total": len(checks), "checks": checks, "metrics": metrics, "full_video_rendered": False, "creative_status": "PENDING_HUMAN", "published": False}
