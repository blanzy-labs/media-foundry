#!/usr/bin/env python3
"""Prove MF-006R3 rejects hierarchy, containment, color, and causality regressions."""
...
def main():
    ...
    mutations={
      ...
      "refresh_after_next_beat":lambda d:d["generated_scene"]["screen_timeline"].update(refresh_1=7.0),
      ...
      "node_competes":lambda d:d["generated_scene"]["projection_emitter"].update(subordinate_to_window=False),
      "cta_overlay":lambda d:d["generated_scene"]["cta"].update(world_integrated=False),
    }
    with tempfile.TemporaryDirectory(prefix="mf006r3-failure-") as td:
        for name,mutate in mutations.items():
            ...
            cmd=["python3",str(Path(a.repo_root)/"scripts/validate_mf006r3_production.py"),"--fixture",a.fixture,"--layout",str(layout),"--execution",a.execution,"--music-stem",a.music_stem,"--music-reference",a.music_reference,"--sfx-audio",a.sfx_audio,"--sfx-report",a.sfx_report,"--mix",a.mix,"--media",a.media,"--contract",a.contract,"--output",str(result),"--motion-timeline",str(motion)]
    ...
    report={"slice":"MF-006R3","case_count":len(cases),"cases":cases,"result":"PASS" if all(c["result"]=="PASS" for c in cases) else "FAIL"}; Path(a.output).write_text(json.dumps(report,indent=2)+"\n"); print(json.dumps(report,indent=2)); return 0 if report["result"]=="PASS" else 1
