#!/usr/bin/env python3
"""Fail-closed approved-copy and indicator-only scope contract for MF-006R9."""
...
def main():
 ...
 try:
  f=json.loads(Path(a.fixture).read_text());root=Path(a.project_root);baseline=json.loads((root/'content/fixtures/mf006r8-unknown-process.json').read_text());s=f.get('subject',{});approved=f.get('approved_copy',{});voice=f.get('voice_contract',{});music=f.get('music',{});types=[x.get('type') for x in f.get('generated_scene',{}).get('components',[])]
  ...
  if f.get('visual_strategy')!={'preference':'godot_indicator_pulse_refinement','fallback':'fail','static_book_cover_allowed':False}:errors.append('STRATEGY_FAILED')
  ...
  if normalized!=baseline:errors.append('R8_CONTENT_SCOPE_FAILED')
  ...
  mapping={'metadata':'METADATA_FAILED','copy':'COPY_FAILED','strategy':'STRATEGY_FAILED','timing':'TIMING_FAILED','single_window':'WINDOW_ARCHITECTURE_FAILED','r8_content_scope':'R8_CONTENT_SCOPE_FAILED','music':'MUSIC_FAILED'};result={'slice':'MF-006R9','checks':{k:'PASS' if v not in errors else 'FAIL' for k,v in mapping.items()}|{'voice':'BLOCKED_PRODUCTION_VOICE'},'blockers':blockers,'errors':errors,'result':'PASS_WITH_BLOCKER' if blockers and not errors else ('PASS' if not errors else 'FAIL')}
 except (OSError,json.JSONDecodeError,TypeError,ValueError,KeyError) as e:result={'slice':'MF-006R9','errors':[str(e)],'blockers':[],'result':'FAIL'}
